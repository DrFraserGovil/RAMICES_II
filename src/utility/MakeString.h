...
// Primary template for basic types (numbers, chars, bools) that can be converted to string
...
struct MakeStringStruct
{
    ...
    static std::string convert(const U& value) {
        std::stringstream ss;
        ss << value;
        return ss.str();
    }
...
struct MakeStringStruct<std::vector<T_Inner>, void> {
    ...
    static std::string convert(const std::vector<T_Inner>& vec, std::string_view delimiter_str) {
        ...
        for (size_t i = 0; i < vec.size(); ++i) {
            ...
            result += MakeStringStruct<T_Inner>::convert(vec[i]);
            if (i < vec.size() - 1) {
...
{
	return MakeStringStruct<T>::convert(obj);
};

template<typename T>
std::string inline MakeString(T obj,std::string delimiter)
{
	return MakeStringStruct<T>::convert(obj,delimiter);
};