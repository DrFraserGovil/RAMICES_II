...
std::vector<std::string> integerGlobalTriggers = {"-log"};
std::vector<int *> integerGlobalPointers = {&opts.Simulation.LoggingLevel};
...
std::vector<std::string> booleanGlobalTriggers = {"-ridges"};
std::vector<bool *> booleanGlobalPointers = {&opts.Element.PrintYieldRidges};
...
bool changeFileRoot(char* arg)
{
	...
	mkdirReturn dirReport = mkdirSafely(FILEROOT);
	prelog.push_back(dirReport.Message);
	if (dirReport.Successful = false)
	{
		return false;
	}
